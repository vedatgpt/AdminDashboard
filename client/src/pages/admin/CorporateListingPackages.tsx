        ...
        <div className="w-full bg-white rounded-lg border border-gray-200 p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <span className="text-[#EC7830] font-medium">Kurumsal İlan Paketleri</span>
            </div>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            {isLoading ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-gray-600">Kurumsal paketler yükleniyor...</span>
              </div>
            ) : error ? (
              <div className="text-center py-8 text-red-600">
                <AlertTriangle className="w-12 h-12 mx-auto mb-3" />
                <p>Kurumsal paketler yüklenirken bir hata oluştu</p>
              </div>
            ) : filteredPackages.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <Package className="w-12 h-12 mx-auto mb-3 text-gray-300" />
                <p>{searchTerm ? 'Paket bulunamadı' : 'Henüz kurumsal paket oluşturulmamış'}</p>
                <p className="text-sm mt-1">
                  {searchTerm 
                    ? 'Arama kriterlerinize uygun paket bulunamadı.' 
                    : 'Başlamak için "Yeni Kurumsal Paket" butonunu kullanın'
                  }
                </p>
                {!searchTerm && (
                  <button
