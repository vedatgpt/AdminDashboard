...
export interface IStorage {
  ...
  getLocationBySlug(slug: string, parentId?: number | null): Promise<Location | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getLocationBySlug(slug: string, parentId?: number | null): Promise<Location | undefined> {
    const conditions = [eq(locations.slug, slug)];
    if (parentId !== undefined) {
      conditions.push(parentId === null ? isNull(locations.parentId) : eq(locations.parentId, parentId));
    }
  ...
  async getChildLocations(parentId: number | null): Promise<Location[]> {
    const condition = parentId === null ? isNull(locations.parentId) : eq(locations.parentId, parentId);
